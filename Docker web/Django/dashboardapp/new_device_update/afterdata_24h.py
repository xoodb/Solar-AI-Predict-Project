...
import json #api 호출
from . import apidb #db호출 파일
from . import get_time #날짜 및 시간 호출 파일
import re
...
# 현재 시간 설정
base_date = get_time.get_base_date2()
base_time = get_time.get_base_time2()
...
# 응답 받은 JSON 데이터 파싱
def get_pridict_data(payload): #데이터중 필요한 데이터만 추출
    result = []
...
#저장된 기기 각각 실행
def new_24_insert(location_code, area):
    upd = 0; ins = 0
    xyvalue = apidb.dbconfreeselect(f'select x, y from xytable where area = "{area}"') #x, y좌표를 가져온다
    payload['nx'] = xyvalue[0][0]
    payload['ny'] = xyvalue[0][1]
    for i in range (2):
        if (i == 1):
            payload['base_date'] = get_time.get_base_date()
            payload['base_time'] = get_time.get_base_time()
        min_temp = 0; max_temp = 0
        data = get_pridict_data(payload) #필요한 데이터만 추출된 값
        ...
        for i in range (len(data)): # 받아온 data의 길이만큼 수행
            ...
            if(stat == 1):
                ...
                info = "err"
    #print("24H FORCAST | ",get_time.get_today("-"), get_time.get_now_time(), "INSERT RATE :", ins, "UPDATE RATE :", upd) #log 확인용
    return