...
from . import sun_imformation #일출 일몰 데이터 가져옴
from . import get_time #시간 제공 파일
...
def get_24hour_data(location_code, date): #24시간 데이터를 일일 데이터로 변환
    count = 0
    wind = 0
    rainy = 0
    snowy = 0
...
def get_day_data(location_code, date, area): # solar_api_5day테이블에서 날짜에 맞는 구름량을 가져온다
...
#등록된 기기의 위치 정보를 가져온다
def new_Day_insert(location_code, area):
    inst = 0; upd = 0
    for i in range(3): 
        get_day_data(location_code ,i, area)
        get_24hour_data(location_code, i)
        ...
        if (stat == 1): # 값이 있으면 UPDATE 실행
            ...
            print("ERROR")
    #print("SORT FORCAST DATA | ",get_time.get_today("-"), get_time.get_now_time(), "INSERT RATE :", inst, "UPDATE RATE :", upd) #log 확인용
    return