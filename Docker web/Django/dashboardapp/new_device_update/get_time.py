...
def get_now_time2(): # 현재 시간을 int형으로 나타내줌
    now_time = (int(hour) * 100) + int(minute)
    return 100 #now_time

def get_base_date2(): # 기상청 단기예보 api 사용을 위한 기준일 설정
    if get_now_time2() < 230:
        yesterday = today - timedelta(1) #timedelta 만큼 전일
        base_date = yesterday.strftime('%Y%m%d')
    else:
        base_date = today.strftime('%Y%m%d')
...
def get_base_time2(): # 기상청 단기예보 api 사용을 위한 base_time 구하기
    base_time = '0200'
    standard_time = get_now_time2()
    if (standard_time >= 2330): #현재 시간에따라 base_time 변경
        base_time = '2300'
    elif (standard_time >= 2030):
        base_time = '2000'
